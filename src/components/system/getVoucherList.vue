<template>
  <div>
    ...
    <div class="page-content">
      <common-header-tilte :title2="'系统赠送抵用券'"></common-header-tilte>
      <div class="header_search">
        <el-row>
          <el-col :span="5">
            <el-select v-model="m_rank" clearable placeholder="请选择券类型" style="width: 90%;" @change="sele2($event)">
              <el-option label="新人" value="new"></el-option>
              <el-option label="正常" value="normal"></el-option>
              <el-option label="休眠" value="sleep"></el-option>
              <el-option label="无效" value="invalid"></el-option>
              <el-option label="伙伴" value="friend"></el-option>
              <el-option label="待激励" value="incentive"></el-option>
            </el-select>
          </el-col>
          <el-col :span="5">
            <el-button type="primary" class="search-btn" @click="search()">搜索</el-button>
            <el-button type="primary" class="search-btn" @click="add()" style="margin-left: 20px;">添加</el-button>
          </el-col>
        </el-row>
      </div>
      <el-table :data="tableData" max-height="450" style="width: 100%;" border v-loading="loading">
        ...
        <el-table-column prop="m_rank" label="类型" min-width="120">
          <template slot-scope="scope">
            <span v-if="scope.row.m_rank == 'new'">新人</span>
            <span v-if="scope.row.m_rank == 'normal'">正常</span>
            <span v-if="scope.row.m_rank == 'sleep'">休眠</span>
            <span v-if="scope.row.m_rank == 'invalid'">无效</span>
            <span v-if="scope.row.m_rank == 'friend'">伙伴</span>
            <span v-if="scope.row.m_rank == 'incentive'">待激励</span>
          </template>
        </el-table-column>
      ...
      <el-dialog :title="dialogTitle" :visible.sync="dialogFormVisible">
        <el-form ref="ruleForm" label-position="right" label-width="100px" class="demo-ruleForm">
          <el-form-item label="选择类型">
            <el-select v-model="edit_m_rank" clearable placeholder="请选择券类型" style="width: 90%;" @change="sele_edit($event)">
              <el-option label="新人" value="new"></el-option>
              <el-option label="正常" value="normal"></el-option>
              <el-option label="休眠" value="sleep"></el-option>
              <el-option label="无效" value="invalid"></el-option>
...
<script>
  ...
  export default {
    ...
    methods: {
      sele_edit (val){
        if (val == '') {
          this.ruleForm.m_rank = ''
        } else {
          this.ruleForm.m_rank = val
        }
      },
      sele2 (val) {
        if (val == '') {
      ...
      getContent (id) {
        ...
        getVoucherDetail({voucher_id: id}).then(function (res) {
          ...
          _this.edit_m_rank = res.data.data.m_rank
          _this.ruleForm.m_rank = res.data.data.m_rank
