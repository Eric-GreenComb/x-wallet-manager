...
<script>
  ...
  import {uploadGoodsPic} from '../../api/index'
  export default {
    props: {
      /*编辑器的内容*/
      value: {
    ...
    methods: {
      ...
      onFileChange(e){
        ...
        data.append('date_time', date.getFullYear() + '-' + (date.getMonth() + 1) + '-' + date.getDate());
        uploadGoodsPic(data).then(function (res) {
          console.log('ppp', res.data.data);
          if(res.data.errcode==0){
            self.editor.insertEmbed(self.editor.getSelection().index, 'image', res.data.data.pic_url);
            self.editor.focus();
