...
<script>
  ...
  export default {
    ...
    created () { // 一进页面就执行方法
      ...
      this.params.id = this.$route.query.id3
      this.params.super_id = this.$route.query.id2
      this.ruleForm.super_id = this.$route.query.id2
      this.ruleForm.token = window.sessionStorage.getItem('session')
      this.ruleForm.goods_id = this.$route.query.id3
    ...
    methods: {
      ...
      getContent (id) {
        var _this = this;
        getDetailSize({type: 1, id: id}).then(function (res) {
